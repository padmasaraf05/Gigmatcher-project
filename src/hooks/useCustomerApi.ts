  ...
  return useQuery<WorkerCard[]>({
    ...
    queryFn: async (): Promise<WorkerCard[]> => {
      ...
      const { data: wpRows } = await supabase
        ...
        .select("user_id, is_available, is_pro, rating, total_reviews, hourly_rate, availability_days")
...
// ─────────────────────────────────────────────────────────────────────────────
// REAL HOOK — Worker Public Profile
// ─────────────────────────────────────────────────────────────────────────────
  ...
  return useQuery<WorkerCard>({
    ...
    queryFn: async () => {
      ...
      const reviews: Review[] = (reviewsRes.data ?? []).map((r) => {
        ...
        return {
          id:           r.id,
          customerName: (reviewer as { full_name?: string } | null)?.full_name ?? "Customer",
          rating:       r.rating,
          comment:      r.comment ?? "",
          date:         dateStr,
        };
  ...
  return useQuery<DashboardData>({
    ...
    queryFn: async () => {
      ...
      const { data: completedJobs } = await supabase
        ...
        .select(`
          worker_id,
          profiles!worker_id ( id, full_name, profile_photo_url, worker_profiles ( rating ) )
        `)
        .eq("customer_id", userId)
        .eq("status", "completed")
        .order("created_at", { ascending: false })
      ...
      for (const job of completedJobs ?? []) {
        ...
        recentWorkers.push({
          ...
          photo: pObj?.profile_photo_url ?? "",
          rating: (wp as { rating?: number } | null)?.rating ?? 0,
          reviewCount: 0, distance: "", rate: 0, skills: [], tools: [],
...
const MOCK_WORKERS: WorkerCard[] = [
  {
    ...
    distance: "1.2 km", rate: 350, skills: ["Electrician", "AC Technician"],
  ...
  },
  {
    ...
    distance: "2.8 km", rate: 280, skills: ["Plumber", "Mechanic"],
    ...
    reviews: [
      { id: "r4", customerName: "Neha R.", rating: 5, comment: "Fixed leaking pipe quickly.", date: "3 days ago" },
    ],
  },
  {
    id: "w-3", name: "Vikram Singh", photo: "", rating: 4.9, reviewCount: 210,
    distance: "3.5 km", rate: 450, skills: ["Carpenter", "Painter"],
    tools: ["Saw", "Hammer", "Measuring Tape", "Chisel Set", "Drill Machine"],
    hasAllTools: true, missingToolCount: 0, available: true,
    availableDays: [false, true, true, true, true, false, true],
    reviews: [
      { id: "r7", customerName: "Deepak T.", rating: 5, comment: "Built amazing custom furniture!", date: "1 day ago" },
    ],
  },
];
